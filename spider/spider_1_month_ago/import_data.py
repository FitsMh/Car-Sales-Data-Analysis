...
from app.models import CarInformationOneMonth
from django.db import IntegrityError

def import_csv_to_db(csv_file_path):
    print(f"开始导入/更新数据: {csv_file_path}")

    CarInformationOneMonth.objects.all().delete()
    print("✅ 表数据已清空，开始重新导入...")
    with connection.cursor() as cursor:
        cursor.execute("ALTER TABLE app_carinformationonemonth AUTO_INCREMENT = 1;")
    ...
    for index, row in df.iterrows():
        try:
            ...
            existing_car = CarInformationOneMonth.objects.filter(
                brand=brand, 
                carname=carname
            ).first()
            ...
            if existing_car:
                ...
                CarInformationOneMonth.objects.create(
                    brand=brand,
                    carname=carname,
                    carimg=row['carimg'],
