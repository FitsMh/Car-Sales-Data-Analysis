...
from app.models import CarInformationHangzhou
...
def import_csv_to_db(csv_file_path):
    ...
    CarInformationHangzhou.objects.all().delete()
    print("✅ 表数据已清空，开始重新导入...")
    with connection.cursor() as cursor:
        cursor.execute("ALTER TABLE app_CarInformationHangzhou AUTO_INCREMENT = 1;")
    ...
    for index, row in df.iterrows():
        try:
            ...
            existing_car = CarInformationHangzhou.objects.filter(
                brand=brand,
                carname=carname
            ).first()
            ...
            if existing_car:
                ...
                CarInformationHangzhou.objects.create(
                    brand=brand,
                    carname=carname,
                    carimg=row['carimg'],
