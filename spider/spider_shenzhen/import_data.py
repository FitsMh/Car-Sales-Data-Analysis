...
from app.models import CarInformationShenzhen
...
def import_csv_to_db(csv_file_path):
    ...
    CarInformationShenzhen.objects.all().delete()
    print("✅ 表数据已清空，开始重新导入...")
    with connection.cursor() as cursor:
        cursor.execute("ALTER TABLE app_CarInformationShenzhen AUTO_INCREMENT = 1;")
    ...
    for index, row in df.iterrows():
        try:
            ...
            existing_car = CarInformationShenzhen.objects.filter(
                brand=brand,
                carname=carname
            ).first()
            ...
            if existing_car:
                ...
                CarInformationShenzhen.objects.create(
                    brand=brand,
                    carname=carname,
                    carimg=row['carimg'],
