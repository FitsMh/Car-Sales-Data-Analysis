...
class spider(object):
    ...
    def main(self, start_offset=0):
        ...
        while self.current_count < self.crawl_count:
            ...
            print(f"第{(current_offset//10)+1}次API请求: offset={current_offset}, count={request_count}, 已爬取={self.current_count}/{self.crawl_count}条")
            ...
            try:
                ...
                for index, car in enumerate(car_list):
                    ...
                    if car_id in self.crawled_ids:
                        print(f"跳过重复车辆：{car.get('series_name')}")
                        continue

                    self.crawled_ids.add(car_id)
