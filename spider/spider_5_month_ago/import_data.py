...
from app.models import CarInformationFiveMonth
...
def import_csv_to_db(csv_file_path):
    ...
    CarInformationFiveMonth.objects.all().delete()
    print("✅ 表数据已清空，开始重新导入...")
    with connection.cursor() as cursor:
        cursor.execute("ALTER TABLE app_CarInformationFiveMonth AUTO_INCREMENT = 1;")
    ...
    for index, row in df.iterrows():
        try:
            ...
            existing_car = CarInformationFiveMonth.objects.filter(
                brand=brand,
                carname=carname
            ).first()
            ...
            if existing_car:
                ...
                CarInformationFiveMonth.objects.create(
                    brand=brand,
                    carname=carname,
                    carimg=row['carimg'],
